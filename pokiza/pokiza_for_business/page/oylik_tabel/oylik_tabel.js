...
// Ma'lumot manbai: pokiza.api.oylik (orqada HRMS: Attendance / SSA / Additional Salary).
...
frappe.pages["oylik-tabel"].on_page_load = function (wrapper) {
	...
	if (!document.getElementById("oylik-tabel-css")) {
		...
		.ot-jadval .ot-c-ism { position:sticky; left:34px; z-index:2; text-align:left; min-width:180px; max-width:220px; overflow:hidden; text-overflow:ellipsis; }
		.ot-jadval .ot-c-oylik { position:sticky; left:214px; z-index:2; min-width:110px; text-align:right; box-shadow:2px 0 3px rgba(0,0,0,.06); }
		.ot-jadval thead .ot-c-nr, .ot-jadval thead .ot-c-ism, .ot-jadval thead .ot-c-oylik { z-index:4; }
	...
	function yukla() {
		frappe.call({
			method: "pokiza.api.oylik.get_tabel",
	...
	function chiz() {
		...
		const $bosh = $(`
			<div class="ot-bosh">
				...
				${badge}
				<span class="ot-info">${d.kun_soni} kun, ish kunlari: <b>${d.ish_kunlari}</b> (yakshanbasiz)</span>
				<div class="ot-qidiruv"><input type="text" placeholder="🔍 Xodim qidirish..." value="${frappe.utils.escape_html(holat.qidiruv)}"></div>
			</div>`);
		...
		// jadval
		let th = `<tr><th class="ot-c-nr">№</th><th class="ot-c-ism">Xodim</th><th class="ot-c-oylik">Oylik (so'm)</th>`;
		d.kunlar.forEach((k) => {
		...
		d.qatorlar.forEach((q, i) => {
			...
			let tr = `<tr data-ism="${frappe.utils.escape_html((q.ismi || "").toLowerCase())}" data-xodim="${q.xodim}">
		...
		const foot = `<tr><td class="ot-c-nr"></td><td class="ot-c-ism">JAMI</td><td class="ot-c-oylik"></td>
		...
		if (d.oylik_izoh_kerak !== false) {
			$joy.append(`<div class="ot-info" style="margin-top:8px">
				* "Kassa taklifi" — oylik hali tasdiqlanmagan, Kassa'dagi oxirgi oy to'lovlaridan avtomatik olingan. Ustiga bosib tasdiqlang yoki o'zgartiring.<br>
				Katak ranglari: <span style="color:#c0392b">qizil — yakshanba (0)</span>, <span style="color:#1a56b0">ko'k — qo'lda o'zgartirilgan</span>, kulrang "1" — avtomatik (to'liq stavka).
			</div>`);
		}
	...
	function saqlaKoef(xodim, sana, koef) {
		frappe.call({
			method: "pokiza.api.oylik.set_koef",
	...
	function oylikDialog(q) {
		...
		const dlg = new frappe.ui.Dialog({
			...
			primary_action: (v) => {
				dlg.hide();
				frappe.call({
					method: "pokiza.api.oylik.set_oylik",
	...
	function bonusDialog(q) {
		const dlg = new frappe.ui.Dialog({
			...
			primary_action: (v) => {
				dlg.hide();
				frappe.call({
					method: "pokiza.api.oylik.set_bonus",
					args: { xodim: q.xodim, yil: holat.yil, oy: holat.oy, summa: v.summa },
					freeze: true,
					callback: () => yukla(),
				});
	...
	function oyOch(d) {
		frappe.confirm(`<b>${d.oy_nomi} ${d.yil}</b> tabelini qayta ochmoqchimisiz?`, () => {
			frappe.call({
				method: "pokiza.api.oylik.oy_och",
				args: { yil: d.yil, oy: d.oy },
				freeze: true,
				callback: () => yukla(),
			});
