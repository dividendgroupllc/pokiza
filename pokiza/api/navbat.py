...
def kun_taqsimla(kelgan_vaqt, jami_kg, exclude_so=None):
...
def so_before_submit(doc, method=None):
    ...
    taqsimot = kun_taqsimla(
        doc.custom_kelgan_vaqt, doc.custom_jami_kg, exclude_so=doc.name
    )
...
def get_navbat(sana=None, gorizont_boshi=None):
    ...
    so_list = (
        frappe.get_all(
            ...
            fields=[
                ...
                "custom_kelgan_vaqt", "custom_mashina_vaqti",
                "custom_mashina_izoh", "custom_izoh", "custom_kg_nomalum",
                "delivery_date",
            ],
        )
        if kun_ulush
    ...
    if so_list:
        rows = frappe.db.sql(
            ...
            SELECT name, parent, item_code, item_name, qty, uom, stock_qty,
                   ...
                   custom_chiqarilgan_vaqt, custom_chiqargan
    ...
    # mahsulot (ГП) kesimi + zames.
    # Bo'lingan zakazda shu kunga to'g'ri kelgan ULUSH proportsional olinadi
    # (qaysi mahsulot qaysi kunda chiqishi kunlik rejada erkin).
    kesim = {}
    for g in tartiblangan:
        for z in g["zakazlar"]:
            ulush = (z["kun_kg"] / z["kg"]) if (z["bolingan"] and z["kg"] > EPS) else 1.0
            for it in z["items"]:
                for gp in it["gps"]:
                    nom = gp["gp"] or _("(retsepti aniqlanmagan)")
                    kesim[nom] = kesim.get(nom, 0.0) + flt(gp["kg"]) * ulush
    kesim_rows = sorted(
        (
